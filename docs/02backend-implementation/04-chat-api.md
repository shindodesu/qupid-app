...
- マッチしたユーザー間での1対1チャット
- メッセージの送信・受信機能
- **音声メッセージ**: 音声ファイルのアップロード・送信（✅実装完了）
- メッセージ履歴の取得
- 既読管理機能
- **リアルタイム通信**: WebSocketによるリアルタイムメッセージ配信（✅実装完了）
- **タイピングインジケーター**: 相手が入力中であることを表示（✅実装完了）
- **オンライン状態管理**: ユーザーのオンライン/オフライン状態の表示（✅実装完了）
- セキュアなコミュニケーション環境の提供
- **モバイルファーストなUI/UX設計**
...
| WS | `/ws` | WebSocket接続（リアルタイム通信） | 必要 |
| WS | `/ws/{conversation_id}` | 会話専用WebSocket接続 | 必要 |
| POST | `/files/upload/voice` | 音声ファイルアップロード | 必要 |
...
4. **ChatWindowコンポーネント** - プロフィール画像・相手名（オンライン状態はUI非表示）
...
### 画像ベースのデザイン要件
- **ヘッダー**: プロフィール画像（グラデーションボーダー）、相手名（オンライン表示はUIから削除済み。送信は下部MessageComposerのみ）
...
- ✅ `ChatWindow` - プロフィール画像・相手名（オンライン状態表示はUIから削除済み）
...
**作成日**: 2024年1月  
**更新日**: 2025年1月（WebSocket、音声メッセージ、オンライン状態管理実装完了）

## 📝 実装完了状況（2025年1月更新）

### ✅ 実装完了機能
- [x] **WebSocketリアルタイム通信**: リアルタイムメッセージ配信、タイピングインジケーター
- [x] **音声メッセージ**: 音声ファイル（MP3, WAV, OGG, M4A）のアップロード・送信
- [x] **オンライン状態管理**: ユーザーのオンライン/オフライン状態の追跡（API・DBは維持。チャット画面のオンライン表示はUIから削除済み）
- [x] **ファイル管理API**: 音声・画像ファイルの安全なアップロード・配信
- [x] **画像ベースUI**: Figmaデザインに基づいた直感的なチャットインターフェース
- [x] **モバイルファースト設計**: レスポンシブ対応、PWA対応
