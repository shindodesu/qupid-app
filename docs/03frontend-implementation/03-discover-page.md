...
### データフロー
1. ページ読み込み時にユーザー一覧を取得（100人）（**すでにいいねしたユーザーはAPI側で除外**）
2. フィルター設定時にAPI呼び出し（フィルター適用）
3. グリッド表示で最大4人のユーザー情報を表示
4. いいね・スキップ時にAPI呼び出し
5. マッチ成立時はバックエンドでトークルームが自動生成される
6. レスポンスに基づいてトースト通知
7. 処理済みユーザーを管理し、新しいユーザーを自動表示
8. 表示可能なユーザーが4人以下になったら自動的に再取得
...
### 2025-02: ユーザーフィードバック対応
- 並び替え: フィルターなし時もsuggestions APIにsortパラメータを渡し、新規順・名前順・人気順を適用
- 探す画面グリッド: 検索・suggestionsレスポンスにavatar_urlを追加し、カードで写真を表示
- フィルター「タグ」: バックエンドでタグ未マッチ時はスキップするよう修正
- プロフィールプレビュー: 「wanting」を「探している関係」に表記統一、戻るボタンを追加
